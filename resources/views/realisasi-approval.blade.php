            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                        ...
                        <th class="p-4 text-center">Bukti Nota</th> <!-- KOLOM BARU BRAY -->
                ...
                <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                    ...
                    <tr class="hover:bg-slate-50/50">
                        ...
                        <td class="p-4 flex gap-2 justify-center">
                            ...
                            <form action="{{ route('realisasi.approval.process', $item->id) }}" method="POST">
                                ...
                                <button type="submit" class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold transition cursor-pointer">
                                    <i class="fa fa-check mr-1"></i> Setuju
                                </button>
                            </form>

                            <!-- Tombol Kelola Tolak -->
                            <button onclick="toggleRejectModal({{ $item->id }})" class="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold transition cursor-pointer">
                                <i class="fa fa-times mr-1"></i> Tolak
                            </button>

                            <!-- Modal Tolak Sederhana -->
                            <div id="modal-reject-{{ $item->id }}" class="hidden fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50">
                                <div class="bg-white p-6 rounded-xl border border-slate-200 w-full max-w-md shadow-xl text-left">
                                    ...
                                    <form action="{{ route('realisasi.approval.process', $item->id) }}" method="POST">
                                        ...
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <button type="submit" class="px-4 py-2 bg-rose-600 text-white rounded-lg text-xs font-bold cursor-pointer">Kirim Tolak</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    @empty
                    <tr>
                        <!-- Diubah ke colspan 6 karena jumlah kolom bertambah bray -->
